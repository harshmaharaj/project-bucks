...
const SliderButton = ({
  ...
  const getMaxPosition = () => {
    ...
    return containerWidth - 68; // 56px for handle width (w-14) + 12px right margin
  };
  ...
  return <div ref={sliderRef} className={cn("relative h-14 overflow-hidden cursor-pointer select-none", variant === 'start' ? "bg-gradient-to-r from-green-500 to-green-600" : "bg-gradient-to-r from-red-500 to-red-600", disabled && "opacity-50 cursor-not-allowed", className)} style={{
    borderRadius: '100px'
  }}>
      {/* Progress Background */}
      ...
      {/* Slider Handle */}
      <div ref={handleRef} className={cn("absolute top-1 left-1 bottom-1 w-14 bg-white shadow-lg", "flex items-center justify-center cursor-grab active:cursor-grabbing", "transition-all duration-200", isDragging && "shadow-xl scale-105", isCompleted && "bg-green-100")} style={{
